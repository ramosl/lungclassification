...
def load_model(img_rows, img_cols):
    ...
    Y_train = truthVals[0:nb_train_samples]
    ...
    for i in range(0,3):
        new_x_valid2[:,:,:,i] = new_x_valid

    new_x_train2 = np.array([cv2.resize(img, (img_rows,img_cols)) for img in new_x_train2[:nb_train_samples,:,:,:]])
    new_x_valid2 = np.array([cv2.resize(img, (img_rows,img_cols)) for img in new_x_valid2[:nb_valid_samples,:,:,:]])
...
if __name__ == '__main__':
    ...
    # model = resnet50_model(224, 224, 3, 10)
